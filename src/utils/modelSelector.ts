...
const AVAILABLE_TRANSFORMERS = [
  'anthropic', 'deepseek', 'gemini', 'openrouter', 'groq', 'maxtoken',
  'tooluse', 'gemini-cli', 'reasoning', 'sampling', 'enhancetool', 'cleancache',
  'vertex-gemini', 'chutes-glm', 'qwen-cli', 'rovo-cli', 'mistral'
];

function getConfigPath(): string {
  const configDir = path.join(
      process.env.HOME || process.env.USERPROFILE || '', '.claude-code-router');
...
async function selectModelType() {
  return await select({
    message:
        `${BOLDYELLOW}Which model configuration do you want to update?${RESET}`,
    choices: [
...
async function addNewModel(config: Config): Promise<ModelResult|null> {
  const providerChoices =
      config.Providers.map(p => ({name: p.name, value: p.name}));

  providerChoices.push(
      {name: `${BOLDGREEN}+ Add New Provider${RESET}`, value: '__new__'});
...
async function addModelToExistingProvider(
    config: Config, providerName: string): Promise<ModelResult|null> {
  ...
  console.log(`${GREEN}✓ Model "${modelName}" added to provider "${
      providerName}"${RESET}`);

  const setAsDefault = await confirm({
    message:
        `\n${BOLDYELLOW}Do you want to set this model in router configuration?${
            RESET}`,
    default: false
  });
...
async function addNewProvider(config: Config): Promise<ModelResult|null> {
  ...
  const setAsDefault = await confirm({
    message: `\n${
        BOLDYELLOW}Do you want to set one of these models in router configuration?${
        RESET}`,
    default: false
  });
  ...
  try {
    ...
    if (action === 'addModel') {
      ...
      if (result) {
        ...
        saveConfig(config);
        console.log(`${GREEN}✓ ${result.modelType} set to ${
            result.providerName},${result.modelName}${RESET}`);
      }
    } else {
