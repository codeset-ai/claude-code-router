...
import {backupConfigFile, checkForUpdates, performUpdate, readConfigFile, writeConfigFile} from './utils';
import {calculateTokenCount} from './utils/router';

export const createServer = (config: any): Server => {
  const server = new Server(config);

  // Register custom transformers immediately after server creation
  // BEFORE onReady hook, so they're available when providers handle requests
  ...
  server.app.get('/api/transformers', async () => {
    ...
    const transformerList = Array.from(transformers.entries())
                                .map(([name, transformer]: any) => ({
                                       name,
  ...
  server.app.get('/api/logs/files', async (req, reply) => {
    try {
      ...
      const logFiles: Array<
          {name: string; path: string; size: number; lastModified: string}> =
          [];
      ...
      if (existsSync(logDir)) {
        ...
        // 按修改时间倒序排列
        logFiles.sort(
            (a, b) => new Date(b.lastModified).getTime() -
